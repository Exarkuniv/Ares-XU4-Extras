...
function sources_nxengine-evo() {
    gitPullOrClone "$md_build" https://github.com/nxengine/nxengine-evo.git master
}
...
function gamedata_nxengine-evo() {
    if [[ ! -f "$romdir/ports/CaveStory/$md_id/Doukutsu.exe" ]]; then
        downloadAndExtract "https://github.com/Exarkuniv/game-data/raw/main/cavestoryen.zip" "$romdir/ports/CaveStory/$md_id"
