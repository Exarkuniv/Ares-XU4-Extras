...
function sources_jfsw() {
    gitPullOrClone "$md_build" https://github.com/jonof/jfsw.git master
}
...
function gamedata_jfsw() {
    ...
    if [[ ! -f "$dest/sw.grp" ]]; then
        ...
        downloadAndExtract ftp://ftp.3drealms.com/share/3dsw12.zip "$md_build"
        cd "$md_build"
        rename 'y/A-Z/a-z/' *
        unzip -Lo "$md_build/swsw12.shr" sw.grp sw.rts -d "$dest"
        rm -rf "$md_build"
    fi
